 ...
 *  ---license-end
 *
 *  Created by osarapulov on 5/13/22, 1:24 AM
 */

package dgca.verifier.app.android.diia
...
import dgca.verifier.app.android.diia.worker.CountriesLoadWorker
import dgca.verifier.app.android.diia.worker.LoadKeysWorker
import dgca.verifier.app.android.diia.worker.RulesLoadWorker
import dgca.verifier.app.android.diia.worker.ValueSetsLoadWorker
...
class DiiaProcessor @Inject constructor(
...
//            schedulePeriodicWorker<ConfigsLoadingWorker>(WORKER_CONFIGS)
    ...
    override fun isApplicable(input: String): Intent? {
        ...
        return Intent("com.android.app.verifier.diia.View", Uri.parse("verifier://diia")).apply {
            putExtra(RESULT_KEY, input)
        }
    }

    override fun getSettingsIntent(): Pair<String, Intent> {
        return Pair("Dcc", Intent("com.android.app.verifier.diia.View", Uri.parse("settings://diia")))
    }
