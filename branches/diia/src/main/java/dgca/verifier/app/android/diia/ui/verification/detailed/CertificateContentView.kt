...
import com.android.app.diia.databinding.ViewCertificateContentViewBinding
import com.google.android.material.card.MaterialCardView
import dgca.verifier.app.android.diia.model.CertificateModel
...
class CertificateContentView(context: Context, attrs: AttributeSet?) :
    ...
    fun setCertificateModel(
        ...
        certAdapter: CertificateContentAdapter
    ) {
        ...
        certificateModel.vaccinations?.let {
            if (it.isNotEmpty()) {
                ...
                data[context.getString(R.string.total_doses_title)] =
                    item.totalSeriesOfDoses.toString()
                data[context.getString(R.string.date_of_vaccination_title)] = item.dateOfVaccination
                data[context.getString(R.string.country_of_vaccination_title)] =
                    item.countryOfVaccination
                data[context.getString(R.string.certificate_issuer_title)] = item.certificateIssuer
                data[context.getString(R.string.certificate_identifier_title)] =
                    item.certificateIdentifier
            }
        }
        certificateModel.tests?.let {
            if (it.isNotEmpty()) {
                ...
                data[context.getString(R.string.test_name_manufacturer_title)] =
                    item.testNameAndManufacturer.orEmpty()
                data[context.getString(R.string.date_time_of_collection_title)] =
                    item.dateTimeOfCollection
                data[context.getString(R.string.date_time_of_test_result_title)] =
                    item.dateTimeOfTestResult.orEmpty()
        ...
        certificateModel.recoveryStatements?.let {
            if (it.isNotEmpty()) {
                ...
                data[context.getString(R.string.target_disease)] = item.disease.value
                data[context.getString(R.string.date_of_first_positive_test_title)] =
                    item.dateOfFirstPositiveTest
                data[context.getString(R.string.country_of_vaccination_title)] =
                    item.countryOfVaccination
                data[context.getString(R.string.certificate_issuer_title)] = item.certificateIssuer
                data[context.getString(R.string.certificate_valid_from_title)] =
                    item.certificateValidFrom
                data[context.getString(R.string.certificate_valid_until_title)] =
                    item.certificateValidUntil
                data[context.getString(R.string.certificate_identifier_title)] =
                    item.certificateIdentifier
            }
        }
